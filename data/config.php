<?php
    ...
    $taskbe = [[$a.' '.$b,$N], [$xOne,$A. ' '. $B. ' '. $C,$xTwo,$xThree], [$num,$Nthree], [implode(" ", $Alist), implode(" ", $Blist_orig)], [matrix($Clist), matrix($Dlist_orig)], [$sevenX.' '.$sevenY, 'x']];
        
    $taskaf = [[$b.' '.$a,round($N/3600)], [$resOne,$resTwo,round($resThree, 2),round($resFour, 2)], [$resSix,$sum], [$Amax, implode(" ", $Blist)], [min($max), matrix($Dlist)], [sevenlab_one($sevenX,$sevenY), 'x']];
        
    $taskdev = [[1,2], [3,4,5,5], [1,2], [4,5], [1,3], [2,6]];
